...
const sort = (numArr) => {
    ...
    for (let i = 0; i < numArr.length; i++) {
        ...
        while (currentVal - 1 !== i) {
            let temp = numArr[currentVal -  1];
            [numArr[i], numArr[currentVal - 1]] = [temp, currentVal];
            currentVal = numArr[i];
            numOfChanges++;
        }
        if (numOfChanges === numArr.length) break;
    }

    return numArr;
};

console.log(sort([3, 1, 5, 4, 2])); // => [1,2,3,4,5]
console.log(sort([2, 6, 4, 3, 1, 5])); // => [1, 2, 3, 4, 5, 6]
console.log(sort([1, 5, 6, 4, 3, 2])); // => [1, 2, 3, 4, 5, 6]
